...
struct Eeprom
{
	struct Saut
	{
		...
		uint8_t S_min;    // Минимальная длина блок-участка (деленная на 10).В БЭК2 не используется, в БЭК не используется начиная с версии 33.
		uint8_t Tip_Loc;  // Тип локомотива.
